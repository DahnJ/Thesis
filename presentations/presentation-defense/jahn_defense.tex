...
Convention: $\varphi(\eta,\gamma)=0$ if $\eta \notin \mathcal D(\gamma)$.

\only<2>{\alert{Assume} $\beamermathcolor{red}\varphi(\eta,\gamma)\neq 0$} 
	For $\mathcal D$, we have that 
	$$\eta \in \mathcal D_\Lambda(\gamma)\iff B(\eta) \cap \Lambda \neq \emptyset.$$ 
	For $\mathcal {LD}$, we obtain 
...
\begin{frame}[noframenumbering]\frametitle{Apollonius problem}
