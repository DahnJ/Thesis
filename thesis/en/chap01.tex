...
\todoo[inline]{Check for punctuation etc, especially in math}
\todoo[inline]{Think about the usage of $\eta$ as synonymous to $\mathrm{conv}(\eta)$. Is it okay?}
...
\draw (-19.93284265814484,32.36947322670743) node[anchor=north west] {\Huge$\sqrt{d(p,q')}$};
...
\draw (-20.89417628569969,28.077006796695088) node[anchor=north west] {\Huge$\sqrt{-d(p,q')}$};
...
\begin{itemize}
	\item $d(q',p) \geq 0$. The point $q'$ lies outside of $B_p$. The quantity $\sqrt{d(p,q')}$ can be understood as the length of the line segment from $q'$ to the point of tangency with $B_p$, see Figure \ref{fig:powerdist}. The power distance is equal to zero precisely when $q'$ lies on the boundary $B_p$.
	\item $d(q',p) < 0$. The point $q'$ lies inside of $B_p$. The quantity $\sqrt{-d(p,q')}$ now describes the length of the segment $q's'$, where $s' \in S_p$ such that the triangle $\Delta p'q's'$ has a right angle $\angle p'q's'$, see Figure \ref{fig:powerdist}. 
\end{itemize}
\end{remark}
