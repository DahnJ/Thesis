...
\unsure{Is it enough to just say this?} Any observation window $\Lambda$ will in general be assumed to have a positive Lebesgue measure.
A subset of $\x$ will be denoted $\eta$. If $\mathrm{card}(\eta)=4$, then $\eta$ is \textit{tetrahedral} or a \textit{tetrahedron}\unsure{Maybe just have tetrahedral, that would kinda solve regularity?}. The symbols $\x'=\mathrm{pr_{\Rt}}(\x), \eta'=\mathrm{pr_{\Rt}}(\eta)$ again refer only to the positional part. 
