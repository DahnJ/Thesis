...
\todoo[inline]{Possibly define notation for spheres and then use it, it might be useful}
...
\end{proposition}
\begin{proof}
	Let $x\in\mathcal F_{lf}$ and $\eta =\{p,q\} \in \text{NNG}(\x)$. WLOG assume that $q$ is the nearest neighbor of $p$. Then $ B(p,\|p-q\|)\cap \x = \{p\}$. Then $\eta$ satisfies the empty ball property with the \note{$x \in B(\eta,\x)$ implies $\|x-a\| < \text{diam}(B(\eta,\x)) = \|p-q\|/2$}circumball $ B(\eta,\x) := B((p+q)/2,\|p-q\|/2) \subset B(p,\|p-q\|)$.
\end{proof}
...
\problem[inline]{But it doesn't exist if it lies inside any of the spheres - it would require a negative weight / imaginary radius}
...
Notice that changing the weight of either of the points ammounts to translation of the hyperplane. 
...
\todoo[inline]{Talk about lifting - additional intuition on how this stuff works}
...
Which contains the quintuples of points which form adjacent tetrahedra in $\mathcal {LD}_4(\x)$. \problem{Yeah but what if the 5 points actually describe 3 tetrahedra, as can be the case? This needs improving}
...
\end{remark}
	...
	\todoo[inline]{Cite CG papers here?}
