...
\subsection{Existence propositions}\label{sec:Existence}
...
\begin{enumerate}[]
	...
	\item \textbf{(U)} We choose $M$ and $\Gamma$ as in section \ref{sec:MGamma}.
		\begin{enumerate}[(U1)]
			\item Same as in proposition \ref{prop:E2}. Although the underlying structure is different, the potential still depends on $\delta(\eta)$ and (\^U1) requires the configuration to have non-infinite energy.
			\item Same as in proposition \ref{prop:E2}, $n_T<56$ and we choose an appropriate $a$.
			\item $\Pi^z_\Lambda(\Gamma)>0$ by remark \ref{r:UA}
		\end{enumerate}
\end{enumerate}
\end{proof}
