...
\begin{itemize}
	\item Marked case: We take $E=\Rt\times S$ where $S=[0,W],W>0$ is the space of marks. The measure on $E$ is $z\lambda \otimes \mu$, where $\mu$ is a non-atomic \unsure{But it doesn't need to be a probability distribution --- it's the intensity. In fact, requiring $\mu$ to be probability would complicate things. Fix this.} probability distribution of marks, $z>0$. 
