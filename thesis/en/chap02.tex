...
\note[inline]{ It'd be nice to comment succintly on why Poisson }
...
\end{definition}
If we use the hypergraph structures $\mathcal D_4$ or $\mathcal {LD}_4$, then we call the resulting GPP a \textit{Gibbs-type tetrahedronization}. 
...
Hereditarity thus gives us a powerful characterization of the Gibbs measure which, as noted previously, is not always available in the Gibbs-type tetrahedronizations defined here. 
 

Luckily, the approach in \cite{DereudreLavancier2009} allows us to directly use GNZ equations even for the non-hereditary case. 
...
Having defined the Gibbs measure, we can now present the approach of \cite{DereudreLavancier2009} in extending the GNZ equations to Gibbs point processes with non-hereditary energy functions. First define 
...
\begin{proposition}
	...
	$$\int 1_{\mathbf N_\infty}(\gamma) \sum_{x \in \gamma} f(x,\gamma -\delta_x) P(d\gamma) = z \int \int f(x,\gamma)e^{-h(x,\gamma)} \tau(dx) P(d\gamma).$$
\end{proposition}
\begin{proof}
