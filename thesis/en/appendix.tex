...
Sadly, for some choices of $+$ and $-$, these equations still seem to be too complex for Mathematica to solve. Luckily, we can simplify them further. 
\begin{enumerate}
	\item Expand the parentheses and subtract $i-$th equation from 1st, where $i=2,3,4$ to obtain three equations linear in $(x,y,z,r)$. 
	\item Solve the linear system to express $x,y,z$ in terms of $r$.
	\item Substitute into the original quadratic equation to obtain a solution for $r$.
	\item Substitute the said $r$ back to the linear system to obtain $x,y,z$.
\end{enumerate}
